...
실시간 채팅 앱 **MOA**의 백엔드 서버. C++ IOCP 기반으로 TCP 소켓 통신, 메시지 라우팅, 파일 관리, 인증, 결제 검증까지 처리한다.

> 클라이언트(React Native)와 함께 1인 개발한 프로젝트입니다.
...
| Auth | bcrypt (password hashing), JWT (session token), OAuth2 (Google/Apple) |
...
- 이모지 반응 (토글 방식, 오프라인 동기화)
- @멘션, 채팅방 공지, 그룹 투표

**인증 / 보안**
- 이메일 + 비밀번호 로그인 (bcrypt)
- 소셜 로그인 — Google, Apple OAuth (Java 검증 서버 연동)
- 세션 토큰 (JWT) + 레이트 리밋 (50 pkt/sec)
- 차단 / 신고

**파일 / 결제**
- GCS Signed URL 기반 파일 업로드 (서버 대역폭 사용 없음)
- 구독 등급별 용량 제한 + 파일 보관 정책 (무료 7일, 구독 영구)
- 만료 파일 자동 삭제 (6시간 주기 백그라운드 배치)
- 인앱결제 영수증 검증 (Google Play / App Store)
...
│  C++ IOCP Server                            │
│                                             │
│  PacketDispatcher ─── 46+ 핸들러            │
│    │                                        │
│    ├── Service Layer                        │
│    │   Auth / Chat / File / Group / Friend  │
│    │   Payment / Block / Notification       │
│    │                                        │
...
TCP 수신 → RecvBuffer 누적 → Length-Prefixed Framing (4B 헤더 + protobuf)
  → PacketDispatcher::DispatchPacket()
    ...
    → switch(payload_case) { 46+ cases }
    → Service 호출 → DB 조회/저장 → 응답 패킷 전송
```
...
│
├── Protobuf/
│   └── protocol.proto     # 60+ 메시지 타입 정의
│
└── database/
    └── db_schema/         # 마이그레이션 SQL (00~20)
```
...
```bash
# Visual Studio에서 cppServer.sln 열기
# Release | x64로 빌드
```

### 설정

`cppServer/config/` 폴더에 아래 파일 필요 (`.gitignore`에 포함, repo에는 없음):

| 파일 | 설명 |
|------|------|
| `server_config.txt` | DB 접속 정보, 리슨 포트 |
| `gcp_key.json` | GCP 서비스 계정 키 (GCS + FCM) |
| `jwt_key.txt` | JWT 서명 시크릿 |

`server_config.txt` 형식:
```
db_host=127.0.0.1
db_port=33060
db_user=root
...
```bash
mysql -u root -p < database/db_schema/00_init.sql
mysql -u root -p chat_server < database/db_schema/01_users.sql
# ... 순서대로 20번까지 실행
```
...
Node.js 기반 부하 테스트 도구로 실제 앱과 동일한 패킷 시퀀스(Login → 동기화 → Heartbeat + Chat + ReadReceipt)를 전송하여 측정.
...
| 접속/로그인 | 100% | 100% | 100% |
| 메시지 실패 | 0 | 0 | - |
| 응답 avg | 2.2ms | 15.2ms | 143ms |
| 응답 p99 | 4ms | 110ms | 272ms |
| 메시지 처리량 | 190 msg/s | **5,198 msg/s** | - |
| 피크 메모리 | 30MB | 497MB | 975MB |
| 정리 후 메모리 | 21MB | 24MB | 25MB |

- 5,000명 동시접속 시 **117만건 메시지 전송, 실패 0**
- 10,000명 동시접속 + 로그인 + 동기화 성공, 크래시 없음
- 세션 종료 후 메모리/핸들 정상 회수 확인 (누수 없음)

---

## 트러블슈팅

### 1. DB 세션 경쟁 상태 — `mysqlx::Session` 단일 공유
...
|------|--------|-------|
| 로그인 성공률 | 79% | **100%** |
| 메시지 성공 | 0 / 8434 실패 | **10458 / 0 실패** |
| 응답 avg | 157ms | **2.2ms** |

### 2. 세션 메모리/핸들 누수 — IOCP 비동기 정리 실패

**증상**: 5,000명 접속 후 전원 disconnect → 핸들 5206, 메모리 503MB 그대로 유지. 반복 시 무한 증가.

**원인**: 클라이언트가 TCP를 먼저 끊으면 `DisconnectEx()` 비동기 완료 통지가 안 와서 `ProcessDisconnect()` → `ReleaseSession()`이 안 불림. `Service::_sessions`(set)의 `shared_ptr` 참조가 안 풀려 소멸자가 안 불림.

**수정**: `CleanupAndRelease()` 메서드로 직접 정리. `DisconnectEx` 비동기 체인 대신 `closesocket()` + `ReleaseSession()` 즉시 호출.
...
`IocpCore::Dispatch()`에서 정리된 세션의 완료 통지 안전 처리:

```cpp
auto owner = iocpEvent->owner;
if (owner)
    owner->Dispatch(iocpEvent, bytes);
// owner가 nullptr이면 이미 정리된 세션 — 무시
```
...
| 프로젝트 | 설명 |
|----------|------|
| [MOA Chat Client](https://github.com/bc-chatapp/chatClient) | React Native 클라이언트 (Private) |
